...
import { ecr20TransactionSimulation } from 'src/domains/RPCRequest/api/fixtures/ecr20TransactionSimulation'
import { erc20Rate } from 'src/domains/RPCRequest/api/fixtures/erc20Rate'
import { erc20TransactionByHash } from 'src/domains/RPCRequest/api/fixtures/erc20TransactionByHash'
import { ecr20TransactionFeeForecast } from 'src/domains/RPCRequest/api/fixtures/erc20TransactionFeeForecast'
import { erc20TransactionReceipt } from 'src/domains/RPCRequest/api/fixtures/erc20TransactionReceipt'
import { erc20TransactionResult } from 'src/domains/RPCRequest/api/fixtures/erc20TransactionResult'
import { ethGetBlockByNumber } from 'src/domains/RPCRequest/api/fixtures/ethGetBlockByNumber'
import { onlyPKAccount } from 'src/domains/Storage/api/fixtures/localStorage'
...
test(`As a user, I should be able to search for an address to send to, so that I can send tokens to any wallet, even without tracking already
    ...
    await userEvent.click(
        await within(sendToModal).findByRole('button', { name: 'Add wallet' })
    )
