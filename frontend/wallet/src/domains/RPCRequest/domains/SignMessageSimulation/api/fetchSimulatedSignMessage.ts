...
const parsePermitAllowance = (
    ...
    object(input).andThen((obj) =>
        shape({
            amount: object(obj.amount).andThen((amountDto) =>
                oneOf([
                    shape({
                        type: match(amountDto.type, 'Limited' as const),
                        amount: parseMoney(amountDto.amount),
                    }),
                    shape({
                        type: match(amountDto.type, 'Unlimited' as const),
                        amount: parseMoney(amountDto.amount),
                    }),
                ])
            ),
        ...
        message: object(input.message).andThen((messageDto) =>
            oneOf([
                ...
                shape({
                    ...
                    allowance: parsePermitAllowance(messageDto.allowance),
                }),
                shape({
                    type: match(
                        messageDto.type,
...
export const fetchSimulatedSignMessage = async ({
    ...
    switch (network.type) {
        case 'predefined':
        case 'testnet':
            return post('/wallet/sign-message/simulate/', {
                body: (() => {
