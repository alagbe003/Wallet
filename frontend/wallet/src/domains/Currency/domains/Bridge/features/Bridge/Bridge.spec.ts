...
} from 'src/domains/Currency/domains/Bridge/api/fixtures/bridgeQuote'
import {
...
} from 'src/domains/Currency/domains/Bridge/api/fixtures/bridgeStatus'
import { buildAllowanceTxEthereumUSDC } from 'src/domains/Currency/domains/Bridge/api/fixtures/buildAllowanceTx'
import {
    buildTxEthereumUSDCPolygonUSDT,
    buildTxPolygonUSDCArbitrumUSDT,
} from 'src/domains/Currency/domains/Bridge/api/fixtures/buildTx'
import { checkAllowanceEthereumUSDC } from 'src/domains/Currency/domains/Bridge/api/fixtures/checkAllowanceEthereumUSDC'
...
import { swapQuotePolygonMaticUSDCNoRoutes } from 'src/domains/Currency/domains/SwapQuote/api/fixtures/swapQuote'
...
beforeEach(() => {
    env = mockEnv()
    originalClose = window.parent.close
    originalOpen = window.open
})

afterEach(() => {
    cleanEnv(env)
    window.parent.close = originalClose
...
test(`As a user I should be able to open bridge with source token specified, so I can swap quicker
    ...
    await waitFor(() => {
        expect(window.parent.close).toHaveBeenCalled()
    })
})
