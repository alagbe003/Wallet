...
import { NetworkHexId, Network, NetworkMap } from '@zeal/domains/Network'
...
const parseSocketRouteUserTx = (
    ...
    object(input).andThen((obj) =>
        shape({
            ...
            approvalData: oneOf([
                object(obj.approvalData).andThen((approvalObj) =>
                    shape({
                        minimumApprovalAmount: bigint(
...
export const fetchBridgeRoutes = async ({
    ...
    const socketRoutes = object(
        await get(
            '/quote',
            {
                excludeDexes: 'zerox', // TODO remove once bug is resolved by socket
                singleTxOnly: true,
