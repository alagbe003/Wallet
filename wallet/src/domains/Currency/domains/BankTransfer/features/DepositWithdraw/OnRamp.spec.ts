...
import {
    unblockEURtoUSDRate,
    unblockGBPtoUSDRate,
} from 'src/domains/Currency/domains/BankTransfer/api/fixtures/exchangeRate'
import {
    onRampCompleted,
    onRampCryptoIssued,
    onRampOutsideTransferReceived,
} from 'src/domains/Currency/domains/BankTransfer/api/fixtures/webhookEvents'
import { testPassword } from 'src/domains/KeyStore/api/fixtures/testPassword'
import { bankTransferOnboarded } from 'src/domains/Storage/api/fixtures/localStorage'
...
test(`As a user, I should be able to setup onramp, so I can learn how to top up my wallet and see bank account details
    ...
    env.api['/wallet/unblock/']['/exchange-rates/'].get = () => [
        200,
        unblockGBPtoUSDRate,
    ]
    ...
    env.api['/wallet/unblock/webhook/'].get = () => [
        200,
        onRampOutsideTransferReceived,
    ]
    await act(() => {
    ...
    env.api['/wallet/unblock/webhook/'].get = () => [200, onRampCryptoIssued]
    await act(() => {
        jest.advanceTimersByTime(10000)
    })

    expect(await screen.findByLabelText('EUR')).toHaveTextContent('-4.76')
    expect(await screen.findByLabelText('USDC')).toHaveTextContent('+5')
    expect(
    ...
    env.api['/wallet/unblock/webhook/'].get = () => [200, onRampCompleted]
    await act(() => {
        jest.advanceTimersByTime(10000)
    })
    ...
    expect(
        await screen.findByRole('progressbar', {
            name: `You've received USDC`,
        })
    ).toHaveAccessibleDescription(
        'Your bank transfer has successfully transferred EUR to USDC.'
    )
