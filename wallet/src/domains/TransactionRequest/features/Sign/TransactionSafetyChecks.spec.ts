...
import { erc20AllowanceError } from 'src/domains/Transactions/domains/SimulatedTransaction/api/fixtures/erc20AllowanceError'
...
test(`As a user I should see is transaction proposed by dApp has passed all safetychecks, so I can feel safety
    ...
    ).toHaveAccessibleDescription('Simulation done using Tenderly')

    expect(
        await within(checksPopup).findByLabelText('Contract is not blacklisted')
    ).toHaveAccessibleDescription('No reports of malicious behaviour')

    expect(
        await within(checksPopup).findByLabelText(
            'USDC is verified by CoinGecko'
        )
    ).toHaveAccessibleDescription('Token is listed on CoinGecko')
