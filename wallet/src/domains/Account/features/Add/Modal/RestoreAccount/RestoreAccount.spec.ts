...
import { localStorageOnboarded } from 'src/domains/Storage/api/fixtures/localStorage'
import { testAccountPK } from 'src/domains/KeyStore/api/fixtures/testAccountPK'
...
test(`As a user I should be able to add account by PK, so I can use this account for transactions
    ...
    const pkInput = await screen.findByPlaceholderText('Type or paste here')
    ...
    expect(pkInput).not.toHaveAttribute('type') //no type attribute in RN
